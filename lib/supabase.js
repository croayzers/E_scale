  ...
  try {
    ...
    if (user.email) {
      ...
      if (invite?.organization_id) {
        await ensureOrganizationMembership(invite.organization_id, user.id, invite.invited_role || 'editor');
        await acceptInvitation(invite.id);
        const invitedOrg = await findOrganizationById(invite.organization_id);
...
async function searchFloorPlans(q, limit = 24, { city, type } = {}) {
  ...
  const rows = await supabaseRest('floor_plans', { query: parts.join('') });
  return Array.isArray(rows) ? rows : [];
}
...
async function searchCommunityFloorPlans(q, limit = 500, { city, type } = {}) {
  ...
  try {
    const rows = await supabaseRest('org_floor_plans', { query: parts.join('') });
  ...
  const rows = await supabaseRest('org_floor_plans', { query: fallback.join('') });
  return Array.isArray(rows) ? rows : [];
}

async function getCommunityFloorPlanFilters() {
  const rows = await supabaseRest('org_floor_plans', { query: '?select=ciudad,tipo&limit=500' });
  if (!Array.isArray(rows)) return { cities: [], types: [] };
...
async function loadCommunityFloorPlan(id) {
  if (!id) return null;
  const rows = await supabaseRest('org_floor_plans', {
...
async function listOrgFloorPlans(orgId) {
  ...
  const base = `?organization_id=eq.${encodeURIComponent(orgId)}&order=created_at.desc&limit=500`;
  try {
    const rows = await supabaseRest('org_floor_plans', {
...
async function saveOrgFloorPlan({ orgId, userId, userName, name, venue, ciudad, tipo, widthM, lengthM, opacity, imageDataUrl }) {
  ...
  const dup = await supabaseRest('org_floor_plans', {
    query: `?select=id&organization_id=eq.${encodeURIComponent(orgId)}&name=eq.${encodeURIComponent(clean)}&limit=1`
  });
  ...
  const rows = await supabaseRest('org_floor_plans', { method: 'POST', body });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}

async function loadOrgFloorPlanById(orgId, id) {
  const rows = await supabaseRest('org_floor_plans', {
    query: `?select=id,name,width_m,length_m,opacity,image_data_url&id=eq.${encodeURIComponent(id)}&organization_id=eq.${encodeURIComponent(orgId)}&limit=1`
  });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}
...
async function listOrgTemplates(orgId, kind) {
  let query = `?organization_id=eq.${encodeURIComponent(orgId)}&order=created_at.desc&limit=200&select=id,name,kind,created_by_display_name,created_at`;
  if (kind) query += `&kind=eq.${encodeURIComponent(kind)}`;
  const rows = await supabaseRest('org_templates', { query });
  return Array.isArray(rows) ? rows : [];
}

async function saveOrgTemplate({ orgId, userId, userName, name, kind, data }) {
  const clean = cleanText(name);
  const dup = await supabaseRest('org_templates', {
    query: `?select=id&organization_id=eq.${encodeURIComponent(orgId)}&kind=eq.${encodeURIComponent(kind)}&name=eq.${encodeURIComponent(clean)}&limit=1`
  });
  if (Array.isArray(dup) && dup.length > 0) return { skipped: true };
  const rows = await supabaseRest('org_templates', {
    method: 'POST',
    body: { organization_id: orgId, created_by_user_id: userId || null, created_by_display_name: userName || null, name: clean, kind, data }
  });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}
...
async function loadOrgTemplateById(orgId, id) {
  const rows = await supabaseRest('org_templates', {
    query: `?select=id,name,kind,data&id=eq.${encodeURIComponent(id)}&organization_id=eq.${encodeURIComponent(orgId)}&limit=1`
  });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}
...
async function deleteOrgTemplate(orgId, id) {
  if (!orgId || !id) return;
  await supabaseRest('org_templates', {
    method: 'DELETE',
    query: `?id=eq.${encodeURIComponent(id)}&organization_id=eq.${encodeURIComponent(orgId)}`
  });
}

async function deleteOrgFloorPlan(orgId, id) {
  if (!orgId || !id) return;
  await supabaseRest('org_floor_plans', {
    method: 'DELETE',
    query: `?id=eq.${encodeURIComponent(id)}&organization_id=eq.${encodeURIComponent(orgId)}`
  });
}
...
async function createCollabSession({ hostUserId, hostDisplayName, sessionName, guestRole, sceneSnapshot, planSnapshot }) {
  ...
  const rows = await supabaseRest('collab_sessions', {
    method: 'POST',
    body: {
...
async function countCollabParticipants(sessionId) {
  ...
  const rows = await supabaseRest('collab_participants', {
...
async function cleanupExpiredCollabSessions() {
  await supabaseRest('collab_sessions', {
    method: 'DELETE',
...
async function findPendingInvitationByEmail(email) {
  ...
  const rows = await supabaseRest('organization_invitations', {
    query: `?invited_email=eq.${encodeURIComponent(email.toLowerCase())}&status=eq.pending&expires_at=gt.${new Date().toISOString()}&limit=1&select=id,organization_id,invited_role`
  });
  return Array.isArray(rows) && rows[0] ? rows[0] : null;
}
...
async function acceptInvitation(invitationId) {
  ...
  await supabaseRest('organization_invitations', {
    method: 'PATCH',
...
async function listOrgInvitations(organizationId) {
  ...
  const rows = await supabaseRest('organization_invitations', {
    query: `?organization_id=eq.${organizationId}&status=eq.pending&select=id,invited_email,invited_role,invited_by_name,created_at,expires_at&order=created_at.desc`
  });
  return Array.isArray(rows) ? rows : [];
}
